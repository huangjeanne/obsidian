...
description: >
  ...
  高级检索页面模拟人类检索行为：选择学术期刊类别、选择来源类别（CSSCI/SCI/北大核心/EI/CSCD等，
  默认CSSCI）、输入主题关键词（含同义词和同位词，用 + 连接）、多组关键词用OR关系连接、
  检索后按被引量排序、切换50条/页、打开摘要视图，最终提取前100篇（不足100篇则全部）
  论文的题录和摘要信息并保存为Excel文件。
  触发条件：用户提到需要在知网/CNKI检索论文、高级检索、按关键词搜索CSSCI/SCI/北大核心论文、
  下载题录信息、获取论文摘要、按被引排序检索；或说"帮我在知网检索XX相关论文"、
  "用知网高级检索搜索XX主题的SCI论文"、"帮我检索XX关键词的北大核心论文"、"检索CSSCI期刊论文"等。
---

# 知网高级检索论文工具

使用Chrome DevTools MCP工具在知网高级检索页面自动执行检索操作，提取指定来源类别期刊论文的题录和摘要信息。

**支持的来源类别**：CSSCI、SCI、北大核心、EI、CSCD、CMSCI、CSSCD等。

## Step 0: 解析用户请求

### 0.1 解析来源类别

从用户输入中识别来源类别，默认为CSSCI：
...
**示例**：
- 用户说"检索SCI论文" → 来源类别 = SCI
- 用户说"帮我在知网检索人工智能相关论文" → 来源类别 = CSSCI（默认）
- 用户说"搜索北大核心期刊" → 来源类别 = 北大核心
...
take_snapshot确认来源类别区域已显示，根据Step 0.1解析的来源类别，找到对应选项的checkbox元素并点击勾选。

**操作步骤**：

1. **取消"全部期刊"**：先取消"全部期刊"的勾选（如已勾选）
2. **勾选目标来源**：根据用户指定的来源类别勾选对应选项

**常见来源类别映射**：

| 用户指定的来源 | 知网页面显示名称 | checkbox文本 |
|---------------|----------------|-------------|
| CSSCI、C刊 | CSSCI | "CSSCI" |
| SCI | SCI | "SCI" |
| 北大核心 | 北大核心 | "北大核心" |
| EI | EI | "EI" |
| CSCD | CSCD | "CSCD" |
| CMSCI、科技核心 | CMSCI | "CMSCI" |

**示例**：
- 用户指定"SCI" → 取消"全部期刊"，勾选"SCI"
- 用户指定"北大核心" → 取消"全部期刊"，勾选"北大核心"
- 用户未指定（默认CSSCI） → 取消"全部期刊"，勾选"CSSCI"

**注意**：如果用户要求多个来源类别（如"CSSCI和北大核心"），则同时勾选多个选项。
